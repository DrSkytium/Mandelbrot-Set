...
#include <stdlib.h>
#include <math.h>
#include <GL/glut.h> // библиотека OpenGL
...
double real_min = -2.9; //левая граница
double real_max = 1.4; //правая граница
double img_min = -1.2; //верхняя граница
double img_max = img_min + (real_max - real_min)*height / width; //нижняя граница
														 
double real_factor = (real_max - real_min) / (width - 1); //реальный множетель
double imag_factor = (img_max - img_min) / (height - 1);  //мнимый множитель

int max = omp_get_max_threads(); //определяем максимальное кол-во потоков
...
void RenderFrame()
{
	...
	//параллельный цикл для вычислений
...
//Определение цвета для показа различных уровней
...
// Инициилизация состояний OpenGL
void init() 
{
	// Настройка текстур
...
void keypress(unsigned char key, int x, int y) 
{
	...
	case 'd':
		...
		printf("Iterations:\t%d\n", iterations);
		break;
		//Увеличь кол-во шагов
	case 'E':
	case 'e':
		step++;
		printf("Step:\t%d\n", step);
		break;
		//Уменьшить кол-во шагов
	case 'Q':
	case 'q':
		if (step > 1) step--;
		printf("Step:\t%d\n", step);
		break;
		//Увеличить кол-во использованных потоков
	case 'L':
	case 'l':
		thread_count += 1;
		printf("Threads:\t%d\n", thread_count);
		break;
		//Уменьшить кол-во использованных потоков
	case 'K':
	case 'k':
